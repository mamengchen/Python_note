...
2**n：表示n次方;
成员运算符特点：一个元素是否在一个元素序列中，返回值是bool类型的
身份运算符：与关系运算符中的==类似，但是==是判断值是否相当，而is是判断两个变量的内存地址是否相等
判断函数类型(isinstance)(a, str)：判断是否类型相同，第二个参数可以填写一个元祖（用于多个类型的判断），type不能判断对象的子类型
表达式是运算符和操作数所构成的序列 ，优先级：and优先于or
优先级从高到低：
**
~ +@ -@(补码，一元加减法)
* / % //
